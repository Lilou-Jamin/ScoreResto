...
function getRestosByTypeCuisine($typeCuisine, $voieAdrR= "", $cpR= "", $villeR="") {
    $resultat = array();

    try {
        $cnx = connexionPDO();
        $req = $cnx->prepare("select * from proposer join typecuisine using(idTC) join resto using(idR) where idTC like :typeCuisine and voieAdrR like :voieAdrR and cpR like :cpR and villeR like :villeR group by idR");
        $req->bindValue(':typeCuisine', $typeCuisine, PDO::PARAM_INT);
        $req->bindValue(':voieAdrR', "%".$voieAdrR."%", PDO::PARAM_STR);
        $req->bindValue(':cpR', $cpR."%", PDO::PARAM_STR);
        $req->bindValue(':villeR', "%".$villeR."%", PDO::PARAM_STR);
        $req->execute();
...
function getRestosByNote() {
    $resultat = array();

    try {
        $cnx = connexionPDO();
        $req = $cnx->prepare("select idR, nomR, numAdrR, voieAdrR, cpR, villeR, latitudeDegR, longitudeDegR, descR, horairesR, avg(note) as notemoyenne from resto join critiquer using(idR) GROUP BY idR ORDER BY notemoyenne DESC limit 4;");
        $req->execute();
