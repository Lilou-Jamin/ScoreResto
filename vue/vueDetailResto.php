...
<?php if(isLoggedOn()){?>
    ...
    <form action="./?action=critiquer&idR=<?= $unResto["idR"];?>" method="POST" class="form_addCom">
        <input type="number" placeholder="Note" min="1" max="5" name="noteCrit">
        <input type="text"  placeholder="Entrez un commentaire..." name="comCrit"><br>
        ...
        <li>
            <span>
                ...
                <?php if ($critiques[$i]["mailU"] == $mailU) { ?>
                    <a href='./?action=suppcritique&idR=<?= $unResto['idR']; ?>'>Supprimer</a>
                <?php } ?>
            </span>
            <div>
                <span>
